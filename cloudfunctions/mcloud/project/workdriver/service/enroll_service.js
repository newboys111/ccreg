...
class EnrollService extends BaseProjectService {
	...
	async viewEnroll(userId, id) {
		...
		if (enrollJoin) {
			...
			enroll.myEnrollJoinTag = (enrollJoin.ENROLL_JOIN_STATUS == EnrollJoinModel.STATUS.WAIT) ? '待审核' : '已报名';
		}
		else {
	...
	async getEnrollList({
		...
		if (cateId && cateId !== '0') where.and.ENROLL_CATE_ID = cateId;
